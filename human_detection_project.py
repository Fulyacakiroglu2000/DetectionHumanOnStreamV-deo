...
cred_obj = firebase_admin.credentials.Certificate(
    "countingpeople-f2000-firebase-adminsdk-qizfa-44872a8ce6.json")
...
posts_ref = db.reference('/').child('timestamped_measures')
...
def motionDetection():
    ...
    while cap.isOpened():
        ...
        if sayac > last_max:
            last_max = sayac
        cur_time = time.time()
        delayed_time = cur_time - last_log_time
        if delayed_time > 5:
            posts_ref.push().set({
        ...
        cv2.putText(frame1, "insan sayisi "+str(sayac), (10, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 1)
        cv2.putText(frame1, 'time square{}'.format(''), (10, 20),
